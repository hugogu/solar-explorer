/**
 * Truncated VSOP87D series for the Earth (Meeus appendix III).
 * Yields the Sun's geometric longitude to about 1", two orders of magnitude
 * better than the abridged series, which matters for equinox instants and for
 * eclipse contact times.
 *
 * Terms are [amplitude, phase, frequency]; amplitudes are 1e-8 rad (L, B) or
 * 1e-8 AU (R) and the argument is evaluated in Julian millennia from J2000.
 */
...
function evaluate(sets: Term[][], tau: number): number {
  let sum = 0;
  for (let i = sets.length - 1; i >= 0; i--) sum = sum * tau + series(sets[i], tau);
  return sum / 1e8;
}

export interface EarthHeliocentric {
  /** heliocentric longitude, radians, mean ecliptic and equinox of date */
  L: number;
...
/** @param jdtt Julian day in Terrestrial Time. */
export function earthVsop87(jdtt: number): EarthHeliocentric {
  const tau = (jdtt - 2451545.0) / 365250;
  return {
    L: evaluate([L0, L1, L2, L3, L4, L5], tau),
    B: evaluate([B0, B1], tau),
    R: evaluate([R0, R1, R2, R3, R4], tau),
  };
}
